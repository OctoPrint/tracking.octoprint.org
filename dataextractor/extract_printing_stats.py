...
QUERY_TOTAL = {
    "aggs": {"1": {"sum": {"field": "payload.elapsed"}}},
    "size": 0,
    "query": {
        "bool": {
            "must": [
                {"range": {"@timestamp": {"gte": CUTOFF, "format": "epoch_millis"}}},
                {"query_string": {"query": "event:print_done"}},
            ]
        },
    },
}
...
QUERY_HISTOGRAM = {
    "aggs": {
        "1": {
            ...
            "aggs": {"2": {"sum": {"field": "payload.elapsed"}}},
        }
    },
    "size": 0,
    "query": {
...
# -- Get total print duration
print("Total duration: Sending query to {}".format(URL))
...
output["total"] = result["aggregations"]["1"]["value"]

# -- Version durations
print("Version durations: Sending query to {}".format(URL))
...
resp = requests.post(url=URL, json=QUERY_HISTOGRAM)
