...
_since = datetime.datetime.fromtimestamp(CUTOFF / 1000).replace(microsecond=0).astimezone().isoformat()
plugins = dict()
...
result = resp.json()
if not "aggregations" in result or not "2" in result.get("aggregations") or not "buckets" in result["aggregations"]["2"]:
    print("No result!")
    sys.exit(-1)
...
for bucket in buckets:
    plugins[bucket["key"]] = dict(instances=bucket["1"]["value"],
                                  versions=dict(),
                                  install_events=0,
                                  uninstall_events=0)
...
result = resp.json()
if "aggregations" in result and "3" in result.get("aggregations") and "buckets" in result["aggregations"]["3"]:
