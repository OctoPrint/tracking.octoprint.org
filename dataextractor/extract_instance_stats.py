...
NOW = time.time()
CUTOFF = int(datetime.datetime.fromtimestamp(NOW - DAYS * 24 * 60 * 60).timestamp() * 1000)
CUTOFF_HISTOGRAM = int(datetime.datetime.fromtimestamp(NOW - DAYS * 24 * 60 * 60).replace(minute=0, second=0, microsecond=0).timestamp() * 1000)
...
output = dict(_since=datetime.datetime.fromtimestamp(CUTOFF / 1000).replace(microsecond=0).astimezone().isoformat())
...
result = resp.json()
if not "aggregations" in result or not "1" in result.get("aggregations") or not "value" in result["aggregations"]["1"]:
    print("No result!")
    sys.exit(-1)
...
versions = dict()
result = resp.json()
if "aggregations" in result and "2" in result.get("aggregations") and "buckets" in result["aggregations"]["2"]:
...
histogram = list()
result = resp.json()
if "aggregations" in result and "1" in result.get("aggregations") and "buckets" in result["aggregations"]["1"]:
    ...
    for bucket in buckets:
        ...
        histogram.append(dict(start=timestamp.isoformat(),
