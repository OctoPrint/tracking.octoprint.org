...
# nginx
NGINX_LOCATION_SOURCE=nginx/tracking.location.conf
NGINX_LOCATION_TARGET=/etc/nginx/snippets/tracking.location.conf
NGINX_LOG_SOURCE=nginx/logformat_tracking.conf
...
# Logstash
LOGSTASH_CONFIG_SOURCE=logstash/logstash-tracking-live.conf
...
sudo cp -v curator/*.yml /etc/curator/
